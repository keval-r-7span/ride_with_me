...
const updateAvailability = async (req, res) => {
  try {
    ...
    await Driver.findByIdAndUpdate(id, { availabilityStatus });
    res.json({ message: 'Availability status of driver updated' });
  } catch (error) {
    console.error('Error updating availability status of driver:', error);
    res.status(500).json({ message: 'update availabiliy error from driverBooking file from controller' });
  }
};
